...
from COVIDNet_CT_master.dataset import COVIDxCTDataset
from COVIDNet_CT_master.data_utils import auto_body_crop
from COVIDNet_CT_master.utils import parse_args
...
def simple_summary(tag_to_value, tag_prefix=''):
    ...
    return tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag=tag_prefix + tag, simple_value=value)
...
def get_lr_scheduler(init_lr, global_step=None, decay_steps=None, schedule_type='cosine'):
    if schedule_type == 'constant':
        ...
        return tf.compat.v1.train.cosine_decay(init_lr, global_step, decay_steps)
    elif schedule_type == 'exp_decay':
        return tf.compat.v1.train.exponential_decay(init_lr, global_step, decay_steps)
...
class COVIDNetCTRunner:
    ...
    def trainval(self, epochs, output_dir, batch_size=1, train_split_file='train.txt', val_split_file='val.txt',
        ...
        with self.graph.as_default():
            ...
            data_next = tf.compat.v1.data.make_one_shot_iterator(dataset).get_next()
            ...
            # Add summaries
            summary_writer = tf.compat.v1.summary.FileWriter(os.path.join(output_dir, 'events'), self.graph)
    ...
    def _get_train_summary_op(self, tag_prefix='train/'):
        loss = self.graph.get_tensor_by_name(LOSS_TENSOR)
        loss_summary = tf.compat.v1.summary.scalar(tag_prefix + 'loss', loss)
        return loss_summary
